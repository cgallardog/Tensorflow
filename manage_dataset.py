...
def create_dataset(train_samples, eval_samples, test_samples, data_path):
    train_dataset = np.empty(shape=(len(train_samples), 288, 5))
    eval_dataset = np.empty(shape=(len(eval_samples), 288, 5))
    test_dataset = np.empty(shape=(len(test_samples), 288, 5))
