...
if modo_extrapolacion == 1:
    ...
    if not os.path.exists(dir_models):
        os.mkdir(dir_models)
all_models = os.listdir(path='OptimizacionParametros/GLU_3_2018')
...
for i in range(len(all_models)):
    ...
    if all_models[i] == 'Models':
        continue
    if all_models[i] =='GLU_3_2018':
        continue
    if all_models[i].find('feat_2') != -1:
        continue
    # cargamos todas las variables necesarias
    conf_path = 'OptimizacionParametros/GLU_3_2018/' + all_models[i] + '/configuracion.txt'
    ...
    model_path = 'OptimizacionParametros/GLU_3_2018/' + model_file + '/' + model_file + '.tf'
    '''
    if extrapolate == 1:
...
all_metrics.to_excel(dir_models + '/mejores_metricas_3_feat_ohio_2018.xlsx', header=True, index=False)
